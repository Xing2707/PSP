...
public class principal{
    ...
    private static final int NUM_CORREDO = 2;

    public static void main(String[] args) throws InterruptedException{
        Object pistora = new Object();
        Thread[] corredores = new Thread[NUM_CORREDO];

        for(int i = 0; i < NUM_CORREDO; i++){
            corredores[i] = new Thread(
                new corredo((int)(Math.random() * NUM_DOCSAL) + NUM_DOCSAL,(int)(Math.random() * TOTAL_CARRERA/2) + (TOTAL_CARRERA),pistora)
            );
        }
